...
import { getCampaignProgress, type CampaignProgress } from "../campaign";
...
interface Row {
  ...
  boxingProgress: CampaignProgress;
  wrestlingProgress: CampaignProgress;
  totalWins: number;
  /** Sum of championship trophies + storybook badges across both sports. */
  totalBadges: number;
}
...
export function FamilyLeaderboard() {
  const rows: Row[] = useMemo(() => {
    ...
    const r = profiles.map(p => {
      ...
      const boxingProgress = getCampaignProgress(p.id, "boxing");
      const wrestlingProgress = getCampaignProgress(p.id, "wrestling");
      return {
        profileId: p.id,
        profileName: p.name,
        ...
        totalBadges: boxingProgress.championshipCount + boxingProgress.storybookCount
                   + wrestlingProgress.championshipCount + wrestlingProgress.storybookCount,
      };
    });
    ...
    <div className="rounded-lg border border-white/10 bg-zinc-900/60 p-3 mt-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.map(r => {
              const champTrophies = "🏆".repeat(Math.min(r.boxingProgress.championshipCount + r.wrestlingProgress.championshipCount, 4));
              const storyBadges = "📖".repeat(Math.min(r.boxingProgress.storybookCount + r.wrestlingProgress.storybookCount, 4));
              return (
              <tr key={r.profileId} className="border-b border-white/5">
                <td className="py-1 pr-2">
                  ...
                  {(champTrophies || storyBadges) && (
                    <span className="ml-1 text-[10px]" title="Championships + Storybooks">
                      {champTrophies}{storyBadges}
                    </span>
                  )}
                </td>
      ...
      <div className="text-[10px] text-white/40 mt-2 leading-snug">
        Records persist per-profile in this browser. Win 5 / 10 / 20 matches to unlock new arena backdrops.
        🏆 = championship win · 📖 = story mode cleared.
      </div>
    </div>
  );
}
