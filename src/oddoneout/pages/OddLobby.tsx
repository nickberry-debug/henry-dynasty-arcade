// Odd One Out — lobby. Host creates, guests join, both pick the word
// pack, everyone taps READY, host starts. Same flow as Crew Traitor.
...
import {
  createRoom, joinRoom, leaveRoom, setReady, setPack, startRound,
  subscribeRoom, heartbeat, normalizeRoomCode, type OddRoom,
} from "../room";
import { PACKS, DEFAULT_PACK } from "../packs";

type Step = "choose" | "join_input" | "lobby";

export default function OddLobby() {
  ...
  useEffect(() => {
    if (room?.status === "playing") {
      navigate(`/odd/game?code=${code}`);
    }
  }, [room?.status, code]);

  useEffect(() => { if (code && step !== "lobby") setStep("lobby"); }, [code, step]);
  ...
  async function pickPack(packId: string) {
    if (!code) return;
    await setPack(code, packId);
  }
  ...
  const activePack = room ? PACKS.find(p => p.id === room.state.packId) ?? PACKS.find(p => p.id === DEFAULT_PACK)! : null;

  return (
    <div className="min-h-screen flex flex-col"
      style={{
        background:
          "radial-gradient(900px 600px at 50% 0%, rgba(244,114,182,0.18), transparent 60%), " +
      ...
      <header className="px-4 py-4 flex items-center gap-3 max-w-3xl mx-auto w-full safe-top">
        ...
        <div className="flex-1">
          <div className="text-[10px] tracking-[0.3em] font-display" style={{ color: "#f9a8d4" }}>🕵️ SOCIAL DEDUCTION</div>
          <h1 className="font-display text-2xl tracking-wider" style={{ color: "#fde047" }}>ODD ONE OUT</h1>
        </div>
      </header>
        ...
        {step === "choose" && (
          <>
            <p className="text-[12px] leading-relaxed text-center" style={{ color: "rgba(229,231,235,0.78)" }}>
              Everyone gets the same secret word except one player — the <b>Odd One Out</b>. Drop a clue, vote out the impostor. 3-8 players.
            </p>
            <button onClick={doCreate} disabled={busy}
              ...
              style={{ background: "linear-gradient(135deg, #f9a8d4, #ec4899)", color: "#0a0a14", minHeight: 64 }}>
              🚀 HOST A NEW ROOM
            </button>
            <button onClick={() => setStep("join_input")}
              className="w-full py-4 rounded-2xl pressable touch-target font-display tracking-widest text-[14px]"
              style={{ background: "rgba(244,114,182,0.10)", border: "1px solid rgba(244,114,182,0.40)", color: "#f9a8d4", minHeight: 64 }}>
              🔑 JOIN WITH CODE
            </button>
          </>
        )}
          ...
          <>
            ...
            <input
              ...
              style={{
                ...
                border: "1.5px solid rgba(244,114,182,0.40)",
                color: "#fde047",
              }}
            />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={doJoin} disabled={busy || joinCode.length < 6}
                className="flex-1 py-3 rounded-xl pressable touch-target text-[12px] font-display tracking-widest"
                style={{ background: "#f9a8d4", color: "#0a0a14", minHeight: 48, opacity: (busy || joinCode.length < 6) ? 0.55 : 1 }}>
                {busy ? "..." : "JOIN"}
              </button>
            </div>
          </>
        ...
        {step === "lobby" && room && (
          <>
            <section className="rounded-2xl p-4 text-center"
              style={{ background: "linear-gradient(135deg, rgba(244,114,182,0.10), rgba(15,8,18,0.85))", border: "1.5px solid #f9a8d4" }}>
              <div className="text-[9px] tracking-[0.4em]" style={{ color: "#f9a8d4" }}>ROOM CODE</div>
              ...
              <div className="text-[10px] mt-2 opacity-75" style={{ color: "rgba(229,231,235,0.7)" }}>
                {playerCount}/8 players · need 3+ to start · share this code
              </div>
            </section>

            {/* Pack picker (host only). Everyone sees the active pack. */}
            <section className="rounded-2xl p-3"
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.10)" }}>
              <div className="flex items-center justify-between mb-2">
                <div className="text-[10px] tracking-[0.3em]" style={{ color: "#f9a8d4" }}>WORD PACK</div>
                {!isHost && (
                  <span className="text-[9px] opacity-60" style={{ color: "rgba(229,231,235,0.7)" }}>(host picks)</span>
                )}
              </div>
              <div className="grid grid-cols-3 gap-1.5">
                {PACKS.map(p => {
                  const active = room.state.packId === p.id;
                  return (
                    <button key={p.id}
                      onClick={() => isHost && pickPack(p.id)}
                      disabled={!isHost}
                      aria-pressed={active}
                      className="rounded-lg p-2 text-center pressable touch-target"
                      style={{
                        background: active ? "rgba(244,114,182,0.25)" : "rgba(255,255,255,0.04)",
                        border: `1px solid ${active ? "#f9a8d4" : "rgba(255,255,255,0.10)"}`,
                        opacity: isHost ? 1 : 0.7,
                      }}>
                      <div className="text-lg">{p.emoji}</div>
                      <div className="font-display text-[10px] tracking-widest" style={{ color: active ? "#f9a8d4" : "#fef3c7" }}>{p.name}</div>
                    </button>
                  );
                })}
              </div>
              {activePack && (
                <div className="text-[10px] mt-2 italic opacity-75" style={{ color: "rgba(229,231,235,0.78)" }}>
            ...
            <section className="rounded-2xl p-3"
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.10)" }}>
              <div className="text-[10px] tracking-[0.3em] mb-2" style={{ color: "#f9a8d4" }}>PLAYERS</div>
